...
const solutions = [
  {
    ...
    description: "Our flagship automation engine uses proprietary AI models to handle complex enterprise processes that traditional RPA cannot touch.",
  ...
  },
  {
    ...
    description: "Connect fragmented data silos and transform raw data into actionable business intelligence using our AI integration layer.",
      ...
      <main className="pt-32">
        <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-20 text-center">
          ...
          <p className="text-xl text-muted-foreground max-w-3xl mx-auto">
            Deep expertise combined with cutting-edge AI to solve your most complex operational challenges.
          </p>
        </section>
        ...
        <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-20 pb-32">
          {solutions.map((sol, index) => (
            <div 
              key={sol.id} 
              id={sol.id}
              className={`flex flex-col lg:flex-row items-center gap-12 ${index % 1 === 1 ? 'lg:flex-row-reverse' : ''}`}
            >
              <div className="lg:w-1/2 glass p-12 rounded-[2.5rem] border-white/10">
                <div className="mb-8 p-4 bg-white/5 w-fit rounded-2xl">
                  {sol.icon}
                </div>
                <h2 className="text-3xl font-display font-bold mb-6">{sol.title}</h2>
                <p className="text-lg text-muted-foreground mb-8">{sol.description}</p>
